/* progit app shell — ported from the design prototype (App3.jsx), wired to real git. */
...
import { useGitActions, useCommitDiff, useLog, useLogStats, useRefs, useRepo, useStatus, useWorkingDiff, useWorktrees } from './api/queries';
...
export default function App() {
  ...
  const logQ = useLog(limit);
  const refsQ = useRefs();
  const statusQ = useStatus();
  const worktreesQ = useWorktrees();

  const statsQ = useLogStats(limit);
  ...
  const commitDiffQ = useCommitDiff(selectedSha);
  const wantWorking = selected === '__wd__';
  const workingDiffQ = useWorkingDiff(wantWorking || peekObj !== null, peekObj?.path);
  ...
  const buildBranchActions = (ref: ViewRef): MenuAction[] => {
    ...
    if (ref.type === 'remote') {
      ...
      out.push({ label: `Fetch '${ref.name}'`, disabled: true, hint: 'M2' });
      return out;
    }
    if (isCurrent) {
      out.push({ label: '✓ current branch', disabled: true });
      out.push({ sep: true });
      out.push({ label: `Push '${ref.name}'`, disabled: true, hint: 'M2' });
    ...
    <div className={'v3' + (detail ? ' drawer-open' : '')}>
      ...
      <div className="v3-top">
        ...
        <div className="v3-actions">
          <button className="tb-btn" disabled title="Coming in M2" style={{ opacity: 0.45, cursor: 'not-allowed' }}>
            <Icon name="fetch" size={13} /> Fetch
          </button>
          <button className="tb-btn primary" disabled title="Coming in M2" style={{ opacity: 0.45, cursor: 'not-allowed' }}>
            <Icon name="push" size={13} /> Push {status && status.ahead > 0 && <span className="cnt">↑{status.ahead}</span>}
          </button>
